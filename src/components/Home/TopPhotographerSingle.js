...
class TopPhotographerSingle extends Component{
  render(){
    let tags = this.props.tags.map((tag) => 
      <a key={tag.toLowerCase()}>{tag}</a>
    );
    return(
      <div>
        <div className="profile-picture"><img className="cover" alt="" src={this.props.img} /></div>
        ...
        <Link className="button" to={"/photographer/"+this.props.id}>Detail</Link>
      </div>
    )
  }
}
