...
export default class Step2DateAvailability extends Component {
  componentDidMount() {
    ...
    function html_scheduleTime() {
      ...
      while (i < 9) html += '<i' + (schedule.indexOf(time[i]) === -1 ? '>' : ' class="active">') + time[i++] + '</i>';
      html += '</div><div>';
      while (i < 16) html += '<i' + (schedule.indexOf(time[i]) === -1 ? '>' : ' class="active">') + time[i++] + '</i>';
      html += '</div></div></div>';
      return html;
    }
    ...
    window.$(function () {
      ...
      window.$('.datepicker.datepicker-dropdown.dropdown-menu').prepend(html_scheduleTime())

      window.$(window).resize(function () {
        window.$('.datepicker.datepicker-dropdown.dropdown-menu').css({top: 0});
      });
    ...
    return (
      <Page>
        <div className="container" id="photographer-landing">
          ...
          <hr/>
          <Link to="/become-our-photographer/step-2-1" className="button button-white-no-shadow u">Back</Link>
          <Link to="/become-our-photographer/step-2-3" className="button">Next</Link>
        </div>
      </Page>
    );
  }
