    ...
    window.$(function () {
          ...
          for (var i = 0, n = Math.min(files.length, 10); i < n; i++) {
            if (files[i]) {
              ...
              reader.onload = function (e) {
                window.$('#photo-preview').append('<div><img src="' + e.target.result + '"><i title="Remove Photo"></i></div>');
              }
              reader.readAsDataURL(files[i]);
            }
          }
          window.$('#photographer-landing > h3').html('Great! You are ready to impress travellers with your work!');
      ...
      photoPreview.on('click', 'i', function () {
        ...
        if (window.$('#photo-preview').children().length === 0) {
          window.$('#photographer-landing > h3').html('Let start building your portofolio! (Maximum 10 photos)');
        ...
        <div className="container" id="photographer-landing">
          ...
          <div className="row">
            <div className="col-sm-7 margin-top-15 margin-bottom-30">
              <div>
                <input accept="image/*" id="upload-file" className="hidden" multiple type="file"/>
                <button className="button" id="upload-btn">Upload</button>
            ...
            <div className="col-sm-5 margin-top-15 margin-bottom-30">
              <div className="card tips">
                ...
                <p>It will be shown on your profile. You can change it later.</p>
          ...
          <hr/>
          <Link to="/become-our-photographer/step-2-2" className="button button-white-no-shadow u">Back</Link>
          <Link to="/become-our-photographer/step-2-4a" className="button">Next</Link>
        </div>
      </Page>
    );
  }
